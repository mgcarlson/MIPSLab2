...
sloop:
    ...
    # Your code begins
      add $t0, $zero, $v0       # [$t0=MaxIndex] MaxIndex ($v0), that needs to be                            swapped with index n - 1
      ...
      #lw $t4, 0($t0)           # [V[$t0]=$t3] Store V[MaxIndex] to be the original                      V[n - 1]
      sw $t3, 0($t0)
